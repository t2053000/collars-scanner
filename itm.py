...
class ItmScanner:
   ...
   def scan_ticker_reverse(self, ticker):
       ...
       Optimizations:
         - REVERSE_DTE_MAX=14 (borrow kills edge beyond 14d)
         - STRIKES_ABOVE_SPOT_REVERSE=2 (nearest 2 strikes capture most edge)
         - Pre-built key lookup dicts (avoids O(n) scan per expiry)
       """
       results = []
       debug = Counter()
       ticker = ticker.upper()
